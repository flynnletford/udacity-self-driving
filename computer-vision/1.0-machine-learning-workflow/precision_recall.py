...
def precision_recall(ious, gt_classes, pred_classes):
    ...
    for x, y in zip(xs, ys):
        if pred_classes[y] == gt_classes[x]:
            tps += 1
        else:
            fps += 1

    # Calculate the false negatives. Here we are basically calcaulting: of all of the possible ground truth objects, which did we miss.
    matches = len(np.unique(xs))
    fns = len(gt_classes) - matches
